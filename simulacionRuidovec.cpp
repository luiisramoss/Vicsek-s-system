...
// Función de simulación con Ruido Vectorial / Intrínseco (Escalado por densidad local)
double simular(int N, double eta, double L, bool guardar_posiciones, string nombre_archivo) {
    ...
    for (int t = 0; t < pasos_totales; t++) {
        ...
        for (int i = 0; i < N; i++) {
            double sS = 0, sC = 0;
            int n_vecinos = 0; // Contador de partículas en el radio de interacción
                ...
                for (int dy_cell = -1; dy_cell <= 1; dy_cell++) {
                    ...
                    for (int j : grid[nx][ny]) {
                        ...
                        if ((dx*dx + dy*dy) <= r*r) {
                            sS += sin(theta[j]);
                            sC += cos(theta[j]);
                            n_vecinos++; // Incrementamos el contador de vecinos
                        }
                    }
                }
            }
          
            // --- IMPLEMENTACIÓN DEL RUIDO VECTORIAL CORRECTAMENTE ESCALADO ---
            double angulo_ruido = (rand() / (double)RAND_MAX) * 2.0 * M_PI;
            
            // El vector de ruido escala proporcionalmente al número de vecinos
            sC += n_vecinos * eta * cos(angulo_ruido); 
            sS += n_vecinos * eta * sin(angulo_ruido);
            
            n_theta[i] = atan2(sS, sC);   // Estimación intrínseca del ángulo
        }
        theta = n_theta;
...
int main() {
    ...
    double rho = 4.0;
    int num_realizaciones = 5; 

    ofstream fout("resultados_va_etavec.txt");
